import {StringHelper as CommonStringHelper} from 'repo-depkit-common';
...
export class StringHelper {
	...
	static replaceAll(str: string, find: string, replace: string) {
		return CommonStringHelper.replaceAll(str, find, replace);
	}
	...
	static replaceAllLineBreaks(str: string, replace?: string) {
		...
		for (let i=0; i<toReplace.length; i++) {
			...
			current = CommonStringHelper.replaceAll(current, find, replace);
		}
		return current;
	}
