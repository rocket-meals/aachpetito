...
import {
  MarkingFilterHelper,
  MarkingWithIdAndExclusionRulesOnly,
} from '../MarkingFilterHelper';
...
const dictMarkingsExclusions = MarkingFilterHelper.getDictMarkingsExclusions([
  marking_exclusion_rule_vegan_is_restricted_by_chicken,
  marking_exclusion_rule_vegan_is_restricted_by_meat,
]);
...
const markings_where_nothing_will_be_filtered_because_no_restrictions = [
  marking_vegan,
  marking_gluten,
  marking_sugar,
];
const markings_where_nothing_will_be_filtered_because_no_other_marking_which_restricts =
  [marking_meat, marking_chicken];
const markings_where_vegan_will_be_filtered = [
  marking_meat,
  marking_vegan,
  marking_gluten,
  marking_chicken,
...
describe('MarkingFilterHelper Test', () => {
  it('Test filterMarkingByRestrictionRules where nothing will be filtered because no restrictions', () => {
    const filteredMarkings =
      MarkingFilterHelper.filterMarkingByRestrictionRules(
        markings_where_nothing_will_be_filtered_because_no_restrictions,
        dictMarkingsExclusions
      );
    expect(filteredMarkings).toEqual(
      markings_where_nothing_will_be_filtered_because_no_restrictions
    );
  });
  ...
  it('Test filterMarkingByRestrictionRules where nothing will be filtered because no other marking which restricts', () => {
    const filteredMarkings =
      MarkingFilterHelper.filterMarkingByRestrictionRules(
        markings_where_nothing_will_be_filtered_because_no_other_marking_which_restricts,
        dictMarkingsExclusions
      );
    expect(filteredMarkings).toEqual(
      markings_where_nothing_will_be_filtered_because_no_other_marking_which_restricts
  ...
  it('Test filterMarkingByRestrictionRules where vegan will be filtered', () => {
    const filteredMarkings =
      MarkingFilterHelper.filterMarkingByRestrictionRules(
        markings_where_vegan_will_be_filtered,
        dictMarkingsExclusions
      );
    expect(filteredMarkings).toEqual([
      marking_meat,
