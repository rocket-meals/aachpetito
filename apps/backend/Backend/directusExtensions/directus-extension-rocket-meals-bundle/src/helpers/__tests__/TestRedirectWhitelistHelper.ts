...
describe('RedirectWhitelistHelper Test', () => {
  it('Redirect url in simple list', async () => {
    ...
    const result =
      RedirectWhitelistHelper.isRedirectUrlAllowedForWhitelistEntriesWithSimpleWildcards(
        redirectWhitelist,
        new URL(redirectUrl)
      );
  ...
  it('Redirect to path', async () => {
    ...
    const redirectWhitelist = [
      'http://localhost/login?directus_refresh_token=1234',
    ];
    const result =
      RedirectWhitelistHelper.isRedirectUrlAllowedForWhitelistEntriesWithSimpleWildcards(
        redirectWhitelist,
  ...
  it('Redirect to path with wildcard', async () => {
    const redirectUrl = 'http://localhost/login?directus_refresh_token=1234';
    const redirectWhitelist = [
      'http://localhost/login?directus_refresh_token=*',
    ];
    const result =
      RedirectWhitelistHelper.isRedirectUrlAllowedForWhitelistEntriesWithSimpleWildcards(
        redirectWhitelist,
  ...
  it('Redirect to path with wildcard not allowed', async () => {
    const redirectUrl =
      'http://localhost/login/?token=directus_refresh_token=1234';
