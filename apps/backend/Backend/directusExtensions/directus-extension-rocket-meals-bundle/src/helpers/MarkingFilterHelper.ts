...
export type DictMarkingsExclusions = Record<
  string,
  DatabaseTypes.MarkingsExclusions
>;
...
export class MarkingFilterHelper {
  static getDictMarkingsExclusions(
    markingsExclusions: DatabaseTypes.MarkingsExclusions[]
  ...
  static filterMarkingByRestrictionRules<
    T extends MarkingWithIdAndExclusionRulesOnly,
  >(markings: T[], dictMarkingsExclusions: DictMarkingsExclusions): T[] {
    ...
    for (let marking_to_be_checked of markings) {
      ...
      let markingExclusionRulesForMarkingIds =
        MarkingFilterHelper.getMarkingExclusionRuleIdsFromMarking(
          marking_to_be_checked
        ); // get the exclusion rules for the marking
      if (markingExclusionRulesForMarkingIds.length > 0) {
        ...
        for (let markingExclusionRuleId of markingExclusionRulesForMarkingIds) {
          ...
          let markingExclusionRule =
            dictMarkingsExclusions[markingExclusionRuleId]; // get the exclusion rule
          if (markingExclusionRule) {
            // if the exclusion rule exists
            let marking_being_restricted_by_id =
              markingExclusionRule.restricted_by_markings_id as string; // get the marking that is restricting the marking
            if (marking_being_restricted_by_id) {
              // if the marking that is restricting the marking exists
              let marking_being_restricted_found_in_present_markings =
                !!markingsDictPresent[marking_being_restricted_by_id]; // check if the marking that is restricting the marking is present in the present markings
  ...
  static getMarkingExclusionRuleIdsFromMarking(
    marking: MarkingWithIdAndExclusionRulesOnly
  ): string[] {
    ...
    let markingExclusionRulesForMarking = marking.excluded_by_markings as
      | any[]
      | DatabaseTypes.MarkingsExclusions[];
    const isArrayOfStringsOrNumbers = markingExclusionRulesForMarking.every(
      item => {
