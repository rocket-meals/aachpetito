...
import {
  EmailOptions,
  MailService as MailServiceType,
} from '@directus/api/dist/services/mail';
...
export default defineHook(async ({ schedule, action, filter }, apiContext) => {
  ...
  filter<Partial<DatabaseTypes.Mails>>(
    CollectionNames.MAILS + '.items.create',
    async (input: Partial<DatabaseTypes.Mails>, meta, eventContext) => {
        ...
        if (input.attachments) {
          ...
          if (attachments_create) {
            for (let attachment of attachments_create) {
              let directus_files_id_raw = attachment.directus_files_id as
                | DatabaseTypes.DirectusFiles
                | string
                | undefined;
        ...
        for (let directus_files_id of directus_files_ids) {
          try {
            ...
            if (send_attachments_as_links) {
              let name =
                direcuts_file.filename_disk ||
                direcuts_file.filename_download ||
                'file';
