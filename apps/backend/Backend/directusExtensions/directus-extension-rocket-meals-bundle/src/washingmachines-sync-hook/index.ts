...
function registerWashingmachinesFilterUpdate(
  apiContext: any,
  registerFunctions: RegisterFunctions
) {
  const { filter } = registerFunctions;
  // Washingmachines Jobs Creation
  filter<DatabaseTypes.Washingmachines>(
    CollectionNames.WASHINGMACHINES + '.items.update',
    async (
      input: DatabaseTypes.Washingmachines,
      { keys, collection },
      eventContext
    ) => {
      ...
      const hasWashingmachineNewPropertyDateFinished =
      ...
      if (
        hasWashingmachineNewPropertyDateFinished &&
        new_date_finished === null
      ) {
        ...
        if (!!washingmachines_ids) {
          for (let washingmachine_id of washingmachines_ids) {
            let washingmachine_curent = await myDatabaseHelper
              .getWashingmachinesHelper()
              .readOne(washingmachine_id);
            ...
            if (!!current_date_stated && !!current_date_finished) {
              ...
              let time_diff =
                new Date(current_date_finished).getTime() -
                new Date(current_date_stated).getTime();
              if (time_diff > 0) {
                let time_hours = parseInt(time_diff / 1000 / 60 / 60 + '');
                let time_minutes = parseInt(
                ...
                const duration_in_minutes = parseInt(
                  time_diff / 1000 / 60 + ''
                ); // Total duration in minutes

                // Round duration to the nearest 10-minute interval
                const duration_rounded_10min_calculated =
                  Math.ceil(duration_in_minutes / 10) * 10;
                ...
                await myDatabaseHelper
                  ...
                  .createOne(partialWashingmachineJob);
              }
            }
          }
        }
...
class WashingmachinesWorkflow extends SingleWorkflowRun {
  ...
  async runJob(
    ...
    const parseSchedule = new WashingmachineParseSchedule(
      workflowRun,
      myDatabaseHelper,
      logger,
      this.usedParser
