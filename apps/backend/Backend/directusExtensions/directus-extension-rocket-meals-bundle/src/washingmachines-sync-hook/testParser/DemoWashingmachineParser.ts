...
export class DemoWashingmachineParser implements WashingmachineParserInterface {
  ...
  async getWashingmachines(): Promise<WashingmachinesTypeForParser[]> {
    ...
    occupiedWashingmachineDateFinished.setMinutes(
      occupiedWashingmachineDateFinished.getMinutes() +
        (5 - (occupiedWashingmachineDateFinished.getMinutes() % 5))
    );

    // washingmachine 1 is occupied if the current minute is 0-5, 10-15, 20-25, ...
    let isWashingMachine1Occupied =
      occupiedWashingmachineDateFinished.getMinutes() % 10 < 5;

    let occupiedWashingmachineExternalIdentifier = isWashingMachine1Occupied
      ? external_identifier_1
      : external_identifier_2;
    let freeWashingmachineExternalIdentifier = isWashingMachine1Occupied
      ? external_identifier_2
      : external_identifier_1;
    ...
    let occupiedWashingmachine: WashingmachinesTypeForParserOmmited = {
      ...
      date_finished: DateHelper.formatDateToIso8601WithoutTimezone(
        occupiedWashingmachineDateFinished
      ),
    };
