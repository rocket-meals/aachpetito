...
export class ParseSchedule {
  ...
  constructor(
    ...
    parser: CashregisterTransactionParserInterface
  ) {
  ...
  async parse(): Promise<Partial<DatabaseTypes.WorkflowsRuns>> {
    try {
      ...
      let myTimer = new TimerHelper(
        'Cash register' + ' parsing',
        totalTransactionsToCheck,
        100
      );
      ...
      for (let i = 0; i < totalTransactionsToCheck; i++) {
        ...
        await this.logger.appendLog(
          'Transaction parsing progress: ' + i + '/' + totalTransactionsToCheck
        );
        ...
        let cashregister_external_id =
          transaction?.cashregister_external_idenfifier;
        //console.log("cashregister_external_id: "+cashregister_external_id);

        let cached_cashregister_id =
          external_cashregister_id_to_internal_cashregister_id[
        ...
        if (cached_cashregister_id === undefined) {
          ...
          let cashRegister = await this.myDatabaseHelper
            .getCashregisterHelper()
            .findOrCreateCashregister(cashregister_external_id);
          if (!!cashRegister) {
            //console.log("cashRegister found: "+cashRegister.id);
            cached_cashregister_id = cashRegister?.id;
            external_cashregister_id_to_internal_cashregister_id[
              cashregister_external_id
            ] = cached_cashregister_id;
        ...
        if (cashregister_id !== undefined) {
          ...
          await this.findOrCreateCashregisterTransaction(
            transaction,
            cashregister_id
          );
        } else {
          console.log(
            'Houston we got a problem? Seems like somebody deleted a cashregister mid transaction'
          );
        }
  ...
  async findOrCreateCashregisterTransaction(
    transaction: CashregistersTransactionsForParser,
    cashregister_id: string
  ) {
    return await this.myDatabaseHelper
      .getCashregisterHelper()
      .findOrCreateCashregisterTransaction(transaction, cashregister_id);
  }
}
