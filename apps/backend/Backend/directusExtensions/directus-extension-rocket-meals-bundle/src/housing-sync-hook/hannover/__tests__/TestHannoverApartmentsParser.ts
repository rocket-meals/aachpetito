...
const html = fs.readFileSync(
  path.resolve(__dirname, './Apartments.html'),
  'utf8'
);
...
describe('Hannover Apartments Parser', () => {
  ...
  it('Get Apartments', async () => {
    ...
    const foundApartment = items.find(
      item =>
        item.basicData.external_identifier === searchApartmentExternalIdentifier
    );
    expect(foundApartment).toBeDefined();
    expect(foundApartment?.buildingData.alias).toBe('Bischofsholer Damm');
    expect(foundApartment?.buildingData.url).toBe(
      'https://www.studentenwerk-hannover.de/wohnen/wohnhaeuser/detailansicht/Bischofsholer%20Damm'
    );
    expect(foundApartment?.buildingData.image_remote_url).toBe(
      'https://www.studentenwerk-hannover.de/fileadmin/user_upload/Bilder/2_Wohnen/WH_BiDa_Herzog.jpg'
    );
  });
});
