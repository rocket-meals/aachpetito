...
export type ScheduleFromExtension = (
  cron: string,
...
export class WorkflowScheduleHelper {
  ...
  static async createWorkflowRunInstance(
    workflowId: string,
  ...
  static async registerScheduleToCreateWorkflowRuns(config: {
    ...
    config.schedule(cronString, async () => {
      try {
        ...
        let workflow = await config.myDatabaseHelper
          .getWorkflowsHelper()
          .readOne(workflowId);
        let enabled = workflow?.enabled;
        if (enabled) {
          // schedule will only create a workflow run if the workflow is enabled
          await WorkflowScheduleHelper.createWorkflowRunInstance(
            config.workflowId,
            config.myDatabaseHelper
          );
        }
      } catch (e) {
        console.error(
          'Error while creating workflow run for workflowId: ' +
            config.workflowId +
            ' for workflow schedule: ' +
            cronString
        );
        console.error(e);
      }
...
export class WorkflowScheduler {
  ...
  static registerWorkflow(
    ...
    if (!!WorkflowScheduler.registeredWorkflows[workflowId]) {
      throw new Error(
        'Workflow with id: ' + workflowId + ' is already registered.'
      );
    }
    WorkflowScheduler.registeredWorkflows[
      workflowRunJobInterface.getWorkflowId()
    ] = workflowRunJobInterface;
  }

  static getRegisteredWorkflow(
    workflowId: string
  ): WorkflowRunJobInterface | undefined {
...
export async function createWorkflowIfNotExisting(
  workflowId: string,
  myDatabaseHelper: MyDatabaseHelper
): Promise<void> {
...
async function getAlreadyRunningWorkflowruns(
  workflowId: string,
  myDatabaseHelper: MyDatabaseHelper
): Promise<DatabaseTypes.WorkflowsRuns[]> {
  let searchWorkflowRuns: Partial<DatabaseTypes.WorkflowsRuns> = {};
  searchWorkflowRuns = {
    workflow: workflowId,
    state: WORKFLOW_RUN_STATE.RUNNING,
  };
  return await myDatabaseHelper
    .getWorkflowsRunsHelper()
    .findItems(searchWorkflowRuns);
}

function getWorkflowIdFromInputWorkflowsRuns(
  input: Partial<DatabaseTypes.WorkflowsRuns>
) {
...
function getDictWorkflowIdToWorkflowRuns(
  ...
  for (let workflowRun of workflowRuns) {
    ...
    if (!!workflowId) {
      let workflowRunsForWorkflow =
        dictWorkflowIdToWorkflowRuns[workflowId] || [];
  ...
  if (input.state === WORKFLOW_RUN_STATE.RUNNING) {
    ...
    for (let workflowId of workflowIds) {
      try {
        ...
        console.error(err);
        throw new Error(
          'modifyInputForCreateOrUpdateWorkflowRunToRunning: Error while create/update of workflowRuns. Cannot find or create workflow with id: ' +
            workflowId
        );
      }
    }
    ...
    for (let workflowId of workflowIds) {
      console.log(
        'Checking if workflow with id: ' + workflowId + ' is enabled'
      );
      let workflow: DatabaseTypes.Workflows | undefined = undefined;
      try {
        workflow = await myDatabaseHelper
          .getWorkflowsHelper()
          .readOne(workflowId);
      } catch (err: any) {
        console.error(err);
        throw new Error(
          'modifyInputForCreateOrUpdateWorkflowRunToRunning: Error while create/update of workflowRuns. Cannot read workflow with id: ' +
            workflowId
        );
      }
      if (!workflow) {
    ...
    let notRegisteredWorkflowIds = workflowIds.filter(
      workflowId => !WorkflowScheduler.getRegisteredWorkflow(workflowId)
    );
    if (notRegisteredWorkflowIds.length > 0) {
      throw new Error(
        '-- No WorkflowRunJobInterface found for workflowIds: ' +
          notRegisteredWorkflowIds.join(', ')
      );
    }
    ...
    for (let workflowId of Object.keys(dictWorkflowIdToWorkflowRuns)) {
      ...
      if (workflowRuns) {
        let alreadyRunningWorkflowRuns = await getAlreadyRunningWorkflowruns(
          workflowId,
          myDatabaseHelper
        );
        let workflowRunJobInterface =
          WorkflowScheduler.getRegisteredWorkflow(workflowId);
        if (!workflowRunJobInterface) {
          // never the case, because we checked before, but just to be sure
          throw new Error(
            '-- No WorkflowRunJobInterface found for workflowId: ' + workflowId
          );
        } else {
          console.log('Handling workflow_runs for workflowId: ' + workflowId);
          let result =
            await workflowRunJobInterface.handleWorkflowRunsWantToRun(
              input,
              workflowRuns,
              alreadyRunningWorkflowRuns
            );
          if (result.errorMessage) {
            console.error(
              'Error while setting workflow_runs to running: ' +
                result.errorMessage
            );
            throw new Error(
              'Error while setting workflow_runs to running: ' +
    ...
    input.log =
      input.log || WorkflowRunLogger.createLogRow('Workflow Run started');
  }

  return input;
}

async function handleActionWorkflowRunUpdatedOrCreated(
  payload: Partial<DatabaseTypes.WorkflowsRuns>,
  myDatabaseHelper: MyDatabaseHelper,
  keys: PrimaryKey[],
  apiContext: any,
  eventContext: any
): Promise<void> {
  await handleActionRunningCreatedOrUpdatedWorkflow(
    payload,
    myDatabaseHelper,
    keys,
    apiContext,
  ...
  await handleActionOnUpdateOrCreateIfWorkflowRunShouldBeDeleted(
    payload,
    myDatabaseHelper,
    keys,
    apiContext,
...
async function handleActionRunningCreatedOrUpdatedWorkflow(
  payload: Partial<DatabaseTypes.WorkflowsRuns>,
  myDatabaseHelper: MyDatabaseHelper,
  keys: PrimaryKey[],
  apiContext: any,
  ...
  if (payload.state === WORKFLOW_RUN_STATE.RUNNING) {
    ...
    let existingWorkflowRuns = await myDatabaseHelper
      .getWorkflowsRunsHelper()
      .readMany(item_ids);
    let dictWorkflowIdToWorkflowRuns = getDictWorkflowIdToWorkflowRuns(
      existingWorkflowRuns
    ) as { [p: string]: DatabaseTypes.WorkflowsRuns[] };
        ...
        if (!workflowRunJobInterface) {
          ...
          for (let workflowRun of workflowRuns) {
            ...
            await myDatabaseHelper
              .getWorkflowsRunsHelper()
              .updateOneWithoutHookTrigger(workflowRun.id, {
            ...
            try {
              ...
              result = await workflowRunJobInterface.runJob(
                workflowRun,
                myDatabaseHelper,
                logger
              );
            ...
            result.runtime_in_seconds = parseInt(
              '' +
                (new Date(result.date_finished).getTime() -
                  new Date(date_started).getTime()) /
                  1000
            );

            await myDatabaseHelper
              .getWorkflowsRunsHelper()
              .updateOneWithoutHookTrigger(workflowRun.id, result);
          }
        }
      }
    }
...
async function handleActionOnUpdateOrCreateIfWorkflowRunShouldBeDeleted(
  payload: Partial<DatabaseTypes.WorkflowsRuns>,
  myDatabaseHelper: MyDatabaseHelper,
  keys: PrimaryKey[],
  apiContext: any,
...
export default defineHook(
  async ({ action, init, filter, schedule }, apiContext) => {
    ...
    init(ActionInitFilterEventHelper.INIT_APP_STARTED, async () => {
      ...
      let workflowRunsRunning = await myDatabaseHelper
        .getWorkflowsRunsHelper()
        .findItems(searchWorkflowRuns);
      ...
      await myDatabaseHelper
        .getWorkflowsRunsHelper()
        .updateManyByItems(workflowsNotFinished, {
          state: WORKFLOW_RUN_STATE.FAILED,
      ...
      let registeredWorkflowsIds =
        WorkflowScheduler.getRegisteredWorkflowsIds();
      for (let workflowId of registeredWorkflowsIds) {
        let searchAndUpdate: Partial<DatabaseTypes.Workflows> = {
          id: workflowId,
          alias: workflowId,
        };
        let workflow = await myDatabaseHelper
          .getWorkflowsHelper()
          .upsertOne(searchAndUpdate);
      }
    });

    // Filter: WorkflowRun created - setzt log, output, date_finished, date_started auf null und state auf "pending"
    filter<Partial<DatabaseTypes.WorkflowsRuns>>(
      CollectionNames.WORKFLOWS_RUNS + '.items.create',
      async (input, { keys, collection }, eventContext) => {
        ...
        if (input.state === WORKFLOW_RUN_STATE.RUNNING) {
          ...
          console.log('modifyInputForCreateOrUpdateWorkflowRunToRunning');
          input = await modifyInputForCreateOrUpdateWorkflowRunToRunning(
            input,
            dictWorkflowIdToWorkflowRuns,
            myDatabaseHelper
          );
    ...
    filter<Partial<DatabaseTypes.WorkflowsRuns>>(
      CollectionNames.WORKFLOWS_RUNS + '.items.update',
      async (input, { keys, collection }, eventContext) => {
        if (input.state === WORKFLOW_RUN_STATE.RUNNING) {
          let item_ids = keys as PrimaryKey[];
          let existingWorkflowRuns = await myDatabaseHelper
            ...
            .readMany(item_ids);
          let dictWorkflowIdToWorkflowRuns =
            getDictWorkflowIdToWorkflowRuns(existingWorkflowRuns);
          ...
          if (inputWorkflowId) {
            ...
            dictWorkflowIdToWorkflowRuns[inputWorkflowId] =
              existingWorkflowRuns;
          }

          let amountDifferentWorkflows = Object.keys(
            dictWorkflowIdToWorkflowRuns
          ).length;
          if (amountDifferentWorkflows > 1) {
            throw new Error(
              'You can only update workflow_runs with the same workflow_id at once.'
            );
          }
          input = await modifyInputForCreateOrUpdateWorkflowRunToRunning(
            input,
    ...
    action(
      CollectionNames.WORKFLOWS_RUNS + '.items.create',
      async (meta, eventContext) => {
        let { payload, key } = meta;
        let keys = [key];
        await handleActionWorkflowRunUpdatedOrCreated(
          payload,
          myDatabaseHelper,
          keys,
          apiContext,
    ...
    action(
      CollectionNames.WORKFLOWS_RUNS + '.items.update',
      async ({ payload, keys }, eventContext) => {
        await handleActionWorkflowRunUpdatedOrCreated(
          payload,
          myDatabaseHelper,
          keys,
