...
export default defineHook(async ({ schedule }, apiContext) => {
  ...
  const cronFrequency = isProduction
    ? cronFrequencyEveryDayAt4AM
    : cronFrequencyEveryMinute;
  ...
  const MAX_MINUTES_TO_KEEP: number = isProduction
    ? MAX_MINUTES_TO_KEEP_PRODUCTION
    : MAX_MINUTES_TO_KEEP_DEVELOPMENT;

  schedule(cronFrequency, async () => {
    apiContext.logger.info(
      SCHEDULE_NAME + ': start schedule run: ' + new Date().toISOString()
    );
    ...
    if (MODE_SELECTED === MODE_DELETE_OLD_LOGS) {
      ...
      apiContext.logger.info(
        SCHEDULE_NAME + ': nowMinusMaxDaysISO: ' + nowMinusMaxDaysISO
      );
