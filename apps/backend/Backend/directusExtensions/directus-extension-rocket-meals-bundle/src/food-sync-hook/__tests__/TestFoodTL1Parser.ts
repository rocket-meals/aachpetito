...
describe('FoodTL1Parser Test', () => {
  it('Get Marking Labels from foodname', async () => {
    ...
    const markingLabelsDict =
      FoodTL1Parser.getMarkingLabelsDictFromFoodName(exampleFoodName);
  ...
  it('Sanitize Foodname from Marking labels', async () => {
    ...
    const sanitizedFoodName =
      FoodTL1Parser.sanitizeFoodNameFromMarkingLabels(exampleFoodName);
  ...
  it('Food attributes parsing', async () => {
    const exampleParsedReportItem: RawTL1FoodofferType = {
      [FoodTL1Parser.DEFAULT_NUTRITIONS_FIELD]:
        'Brennwert=612 kJ (146 kcal), Fett=1,1g, davon gesättigte Fettsäuren=0,6g, Kohlenhydrate=19,8g, davon Zucker=18,8g, Ballaststoffe=0,0g, Eiweiß=12,8g, Salz=0,1g,',
    };

    const parsedFoodAttributes =
      FoodTL1Parser.getFoodNutritionAttributeValuesFromRawTL1Foodoffer(
        exampleParsedReportItem
      );
    ...
    for (let parsedFoodAttribute of parsedFoodAttributes) {
      ...
      dictOfFoundExternalIdentifiers[parsedFoodAttribute.external_identifier] =
        true;
      let expectedAttributeValue =
        expectedExternalIdentifiersAndValues[
          parsedFoodAttribute.external_identifier
        ];
      expect(expectedAttributeValue).not.toBeNull();
      expect(expectedAttributeValue).not.toBeUndefined();
      expect(JSON.stringify(parsedFoodAttribute.attribute_value)).toStrictEqual(
        JSON.stringify(expectedAttributeValue)
      );
    ...
    for (let expectedExternalIdentifier of Object.keys(
      expectedExternalIdentifiersAndValues
    )) {
