...
function generateFoodId(
  foodIds: number[],
  markingExternalIdentifiers: string[]
): string | null {
  return FoodTL1ParserHannover.getHannoverFoodIdByRecipeIdsAndMarkings(
    foodIds,
    markingExternalIdentifiers
  );
}

async function getFoodoffersJson(
  reportToReturn?: string | undefined,
  markingsJSONList?: MarkingsTypeForParser[] | undefined
) {
  let testFileGetter: FoodTL1Parser_GetRawReportInterface =
    new FoodTL1Parser_RawReportTestReaderHannover(reportToReturn);
...
describe('FoodTL1ParserHannover Test', () => {
  let testFileGetter: FoodTL1Parser_GetRawReportInterface =
    new FoodTL1Parser_RawReportTestReaderHannover();
  let foodParser: FoodTL1Parser = new FoodTL1ParserHannover(testFileGetter);
  ...
  function checkIfFoodHasMarking(
    foodId: string | null,
    expectedMarkingExternalIdentifiersToBeIncluded: string[],
    foodOffersJson: FoodoffersTypeForParser[]
  ) {
    ...
    for (let foodOffer of foodOffersWithSpecialMarking) {
      ...
      expect(foodOffer.marking_external_identifiers).toEqual(
        expect.arrayContaining(expectedMarkingExternalIdentifiersToBeIncluded)
      );
    }
  }
  ...
  it('Markings use correctly addition field for vegetarian (v)', async () => {
    ...
    let findFoodId = generateFoodId(
      [801346, 802285],
      marking_external_identifiers
    );
    const expectedMarkingExternalIdentifiersToBeIncluded = ['v'];
    let foodOfferJson = await getFoodoffersJson(
      FoodTL1Parser_RawReportTestReaderHannover.getSavedRawReportWithVegatarian()
    );
    checkIfFoodHasMarking(
      findFoodId,
      expectedMarkingExternalIdentifiersToBeIncluded,
      foodOfferJson
    );
  });
  ...
  it('Hannover Food Id only contains existing marking ids', async () => {
    ...
    let foodOfferJson = await getFoodoffersJson(
      FoodTL1Parser_RawReportTestReaderHannover.getSavedRawReportWithNonExistingMarking(
        additionalMarkingsString
      ),
      markingTypeForParser
    );
    ...
    let expectedFoodId = generateFoodId(
      [801346],
      filteredRawMarkingExternalIdentifiers
    );

    for (let foodOffer of foodOfferJson) {
      expect(foodOffer.marking_external_identifiers).toEqual(
        expect.arrayContaining(filteredRawMarkingExternalIdentifiers)
      );
  ...
  it('Markings for co2 values are correctly set', async () => {
    let foodOfferJson = await getFoodoffersJson(
      FoodTL1Parser_RawReportTestReaderHannover.getSavedRawReportWithCO2RatingValues()
    );
    expect(!!foodOfferJson).toBe(true);
    expect(foodOfferJson.length).toBeGreaterThan(0);
    const expectedMarkingExternalIdentifiers = [
      FoodTL1ParserHannover.getCO2RatingMarkingExternalIdentifier('A'),
    ];
    for (let foodOffer of foodOfferJson) {
      expect(foodOffer.marking_external_identifiers).toEqual(
        expect.arrayContaining(expectedMarkingExternalIdentifiers)
      );
    }
  });
  ...
  it('Markings use correctly additional field for vegan (x)', async () => {
    ...
    let findFoodId = generateFoodId(
      [800562, 802726, 801834, 801454],
      marking_external_identifiers
    );
    let foodOfferJson = await getFoodoffersJson(
      FoodTL1Parser_RawReportTestReaderHannover.getSavedRawReportWithVegan()
    );
  ...
  it('Markings correctly used the ZS_NUMMERN Field', async () => {
    let foodOfferJson = await getFoodoffersJson(
      FoodTL1Parser_RawReportTestReaderHannover.getSavedRawReportWithMultipleFoodoofersWithCorrectMarkingsInFieldZS_NUMMER()
    );
    ...
    expect(firstFoodOffer.marking_external_identifiers).toEqual(
      expect.arrayContaining(expectedMarkingExternalIdentifiers)
    );
  });
  ...
  it('Foodoffer has correct category', async () => {
    let foodOffersJson = await getFoodoffersJson(
      FoodTL1Parser_RawReportTestReaderHannover.getSavedRawReportWithFoodofferCategoryMeat()
    );
    let firstFoodOffer = foodOffersJson[0];
    expect(!!firstFoodOffer).toBe(true);
    if (firstFoodOffer) {
      expect(firstFoodOffer.category_external_identifier).toBe(
        'FLEISCH & MEER'
      );
    }
  });
  ...
  it('Food offers with same recipe ids and same markings shall have same food ids', async () => {
    let foodOfferJson = await getFoodoffersJson(
      FoodTL1Parser_RawReportTestReaderHannover.getSavedRawReportWithMultipleFoodoofersSameMarkings()
    );
    let foodsJson = await getFoodsJson(
      FoodTL1Parser_RawReportTestReaderHannover.getSavedRawReportWithMultipleFoodoofersSameMarkings()
    );
  ...
  it('Food offers with CO2_Rating A should have KlimaTeller Marking', async () => {
    ...
    let foodOfferJson = await getFoodoffersJson(
      FoodTL1Parser_RawReportTestReaderHannover.getSavedRawReportWithCO2WithRatingValue(
        co2_rating_value
      )
    );
    let expectedMarkingExternalIdentifiers = [
      FoodTL1ParserHannover.getCO2RatingMarkingExternalIdentifier(
        co2_rating_value
      ),
      FoodTL1ParserHannover.KLIMA_TELLER_EXTERNAL_IDENTIFIER,
    ];
    expect(!!foodOfferJson).toBe(true);
    expect(foodOfferJson.length).toBeGreaterThan(0);
    for (let foodOffer of foodOfferJson) {
  ...
  it('Food offers with CO2_Rating other than A should not have automatically KlimaTeller Marking', async () => {
    ...
    let notExpectedMarkingExternalIdentifiers = [
      FoodTL1ParserHannover.KLIMA_TELLER_EXTERNAL_IDENTIFIER,
    ];
    expect(!!foodOfferJson).toBe(true);
    expect(foodOfferJson.length).toBeGreaterThan(0);
    for (let foodOffer of foodOfferJson) {
      expect(foodOffer.marking_external_identifiers).not.toEqual(
        expect.arrayContaining(notExpectedMarkingExternalIdentifiers)
      );
    }
  });
  ...
  it('Food offers with same recipe ids and differnet markings shall have different food ids', async () => {
    let foodOfferJson = await getFoodoffersJson(
      FoodTL1Parser_RawReportTestReaderHannover.getSavedRawReportWithMultipleFoodoofersDifferentMarkings()
    );
    let foodsJson = await getFoodsJson(
      FoodTL1Parser_RawReportTestReaderHannover.getSavedRawReportWithMultipleFoodoofersDifferentMarkings()
    );
  ...
  function checkForAllAttributesPassed(
    foodOrFoodoffersJson:
      | FoodoffersTypeForParser[]
      | FoodsInformationTypeForParser[],
    allAttributeFields: Tl1AttributeType[]
  ) {
    ...
    for (let attributeField of allAttributeFields) {
      dictAllAttributeFields[attributeField.external_identifier] =
        attributeField;
    }
    ...
    for (let foodOrFoodoffer of foodOrFoodoffersJson) {
      ...
      for (let foodofferAttribute of foodofferAttributes) {
        dictOfFoodOrFoodofferAttributes[
          foodofferAttribute.external_identifier
        ] = foodofferAttribute;
      }
      ...
      for (let externalIdentifier of externalIdentifiers) {
        ...
        let foundAttribute =
          dictOfFoodOrFoodofferAttributes[externalIdentifier];
        expect(!!foundAttribute).toBe(true);
        if (!foundAttribute) {
          return;
        }

        expect(foundAttribute.external_identifier).toBe(
          expectedAttribute.external_identifier
        );

        // Now check if the value is correct set
        if (expectedAttribute.value_type === TL1AttributeValueType.NUMBER) {
          expect(
            foundAttribute.attribute_value.number_value
          ).not.toBeUndefined();
        } else if (
          expectedAttribute.value_type === TL1AttributeValueType.STRING
        ) {
          expect(
            !!foundAttribute.attribute_value.string_value
          ).not.toBeUndefined();
        } else if (
          expectedAttribute.value_type === TL1AttributeValueType.BOOLEAN
        ) {
          expect(
            !!foundAttribute.attribute_value.boolean_value
          ).not.toBeUndefined();
        }
      }
    }
  ...
  it('Food offers attributes are all parsed', async () => {
    let foodoffersJson = await getFoodoffersJson(
      FoodTL1Parser_RawReportTestReaderHannover.getSavedRawReportWithAttributeValues()
    );
