...
export class FoodTL1ParserHannover extends FoodTL1Parser {
   ...
   * @param markings
   */
  _filterZsNummernOnlyForPassedExternalMarkingIdentifiersFromMarkingParser(
    markings: string[]
  ) {
  ...
  _getMarkingsExternalIdentifiersFromRawFoodoffer(
    raw_tl1_foodoffer_json: RawTL1FoodofferType
  ): string[] {
    let tl1_zusatz_nummern_string =
      raw_tl1_foodoffer_json[FoodTL1Parser.DEFAULT_ZSNUMMERN_FIELD];

    let tl1_menuekennzeichen_string =
      raw_tl1_foodoffer_json[FoodTL1ParserHannover.MENUEKENNZEICHEN_FIELD];
    let tl1_co2_bewertung_string =
      raw_tl1_foodoffer_json[FoodTL1ParserHannover.EXTINFO_CO2_BEWERTUNG];
    ...
    if (!!tl1_zusatz_nummern_string) {
      ...
      markings =
        this._filterZsNummernOnlyForPassedExternalMarkingIdentifiersFromMarkingParser(
          markings
        );
      combinedMarkings = combinedMarkings.concat(markings);
    }
    if (!!tl1_menuekennzeichen_string) {
      let markings = tl1_menuekennzeichen_string
        .split(',')
        .map(nummernString => {
          return nummernString.trim();
        });
    ...
    if (!!tl1_co2_bewertung_string) {
      ...
      if (
        tl1_co2_bewertung_string === FoodTL1ParserHannover.CO2RATING_A_VALUE
      ) {
        combinedMarkings.push(
  ...
  override getMarkingsExternalIdentifiersFromRawFoodoffer(
    ...
    return this._getMarkingsExternalIdentifiersFromRawFoodoffer(
      rawFoodoffer.raw_tl1_foodoffer_json
    );
  }
  ...
  static getCO2RatingMarkingExternalIdentifier(co2_bewertung_string: string) {
    return (
      FoodTL1ParserHannover.CO2_BEWERTUNG_PREFIX_IDENTIFIER +
      co2_bewertung_string
    );
  }

  static getCombinedSortedMarkingsExternalIdentifiersAsString(
    total_marking_external_identifier_list: string[]
  ) {
    let sorted_marking_external_identifiers =
      total_marking_external_identifier_list.sort();
    let combined_marking_ids_as_string =
      sorted_marking_external_identifiers.join('-');
    return combined_marking_ids_as_string;
  }

  static getHannoverFoodIdByRecipeIdsAndMarkings(
    recipe_ids: string[] | number[],
    marking_ids: string[]
  ) {
    let sorted_recipe_ids =
      FoodTL1Parser.getSortedRecipeIdFromListOfRecipeIds(recipe_ids);
    let combined_marking_ids_as_string =
      FoodTL1ParserHannover.getCombinedSortedMarkingsExternalIdentifiersAsString(
        marking_ids
      );
    let food_id = sorted_recipe_ids;
    if (
      !!combined_marking_ids_as_string &&
      combined_marking_ids_as_string.length > 0
    ) {
  ...
  override getFoodId(
    ...
    let total_marking_external_identifier_list =
      this._getMarkingsExternalIdentifiersFromRawFoodoffer(
        firstRawTL1Foodoffer
      );

    let food_id = FoodTL1ParserHannover.getHannoverFoodIdByRecipeIdsAndMarkings(
      recipe_ids,
      total_marking_external_identifier_list
    );
  ...
  override getFoodAttributesFromRawTL1Foodoffer(
    ...
    let foodAttributes =
      FoodTL1Parser.getAdditionalFoodAttributesFromRawTL1Foodoffer(
        parsedReportItem,
        FoodTL1ParserHannover.FOOD_ATTRIBUTE_FIELDS
      );
