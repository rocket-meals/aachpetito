...
export class FoodTL1Parser implements FoodParserInterface {
  ...
  static DEFAULT_NUTRITION_FIELD_BRENNWERT_EXTERNAL_IDENTIFIER =
    'calories_kcal';
  static DEFAULT_NUTRITION_FIELD_FAT_EXTERNAL_IDENTIFIER = 'fat_g';
  static DEFAULT_NUTRITION_FIELD_SATURATED_FAT_EXTERNAL_IDENTIFIER =
    'saturated_fat_g';
  static DEFAULT_NUTRITION_FIELD_CARBOHYDRATE_EXTERNAL_IDENTIFIER =
    'carbohydrate_g';
  ...
  protected markingsJSONListFromMarkingParger:
    | MarkingsTypeForParser[]
    | undefined = undefined; // passed from MarkingParserInterface
  ...
  public async createNeededData(
    markingsJSONList?: MarkingsTypeForParser[] | undefined
  ) {
    this.resetData();
    ...
    this.rawFoodoffersJSONList =
      await this.getRawFoodofferJSONListFromRawReport(rawReport);
  }
  ...
  async getFoodsListForParser() {
    let foodIdToRawFoodofferDict = FoodTL1Parser.getFoodIdToRawFoodofferDict(
      this.rawFoodoffersJSONList
    );
    ...
    for (let foodId of foodIds) {
      ...
      if (!!rawFoodoffer) {
        let foodInformationForParser =
          this.getFoodInformationFromRawFoodoffer(rawFoodoffer);
  ...
  getFoodCategoryFromRawFoodoffer(
  ...
  getFoodInformationFromRawFoodoffer(
    rawFoodoffer: RawFoodofferInformationType
  ): FoodsInformationTypeForParser | null {
    ...
    let foodAttributes =
      this.getFoodAttributesFromRawTL1Foodoffer(parsedReportItem);
    ...
    return {
      ...
      category_external_identifier:
        this.getFoodCategoryFromRawFoodoffer(rawFoodoffer),
      marking_external_identifiers:
        this.getMarkingsExternalIdentifiersFromRawFoodoffer(rawFoodoffer),
    };
  }
  ...
  async getCanteensList(): Promise<CanteensTypeForParser[]> {
    ...
    for (let rawFoodoffer of rawFoodoffers) {
      let parsedReportItem =
        FoodTL1Parser.getParsedReportItemFromrawFoodoffer(rawFoodoffer);
      let canteenLabel =
        FoodTL1Parser.getCanteenLabelFunction(parsedReportItem);
      if (!!canteenLabel) {
  ...
  getFoodofferCategoryFromRawFoodoffer(
    rawFoodoffer: RawFoodofferInformationType
  ): string | null {
    let parsedReportItem =
      FoodTL1Parser.getParsedReportItemFromrawFoodoffer(rawFoodoffer);
    return (
      parsedReportItem?.[FoodTL1Parser.DEFAULT_FOODOFFER_CATEGORY_FIELD] || null
    );
  }
  ...
  async getFoodoffersForParser(): Promise<FoodoffersTypeForParser[]> {
    ...
    for (let rawFoodoffer of rawFoodoffers) {
      ...
      const basicFoodofferData: FoodofferTypeWithBasicData = {
        alias: FoodTL1Parser._getFoodNameDe(parsedReportItem),
        price_employee: FoodTL1Parser.getPriceForGroup(
          parsedReportItem,
          PriceGroupEnum.PRICE_GROUP_EMPLOYEE
        ),
        price_guest: FoodTL1Parser.getPriceForGroup(
          parsedReportItem,
          PriceGroupEnum.PRICE_GROUP_GUEST
        ),
        price_student: FoodTL1Parser.getPriceForGroup(
          parsedReportItem,
          PriceGroupEnum.PRICE_GROUP_STUDENT
        ),
      };
      ...
      const foodofferForParser: FoodoffersTypeForParser = {
        ...
        attribute_values: foodAttributes,
        category_external_identifier:
          this.getFoodofferCategoryFromRawFoodoffer(rawFoodoffer),
        marking_external_identifiers:
          this.getMarkingsExternalIdentifiersFromRawFoodoffer(rawFoodoffer),
  ...
  filterZsNummernOnlyForPassedExternalMarkingIdentifiersFromMarkingParser(
    markings: string[]
  ) {
    let markingForParserFromMarkingParser =
      this.markingsJSONListFromMarkingParger;
    if (!!markingForParserFromMarkingParser) {
      let markingExternalIdentifiersFromMarkingParser =
        markingForParserFromMarkingParser.map(marking => {
  ...
  getMarkingsExternalIdentifiersFromRawFoodoffer(
    rawFoodoffer: RawFoodofferInformationType
  ): string[] {
    let parsedReportItem =
      FoodTL1Parser.getParsedReportItemFromrawFoodoffer(rawFoodoffer);
    ...
    let marking_external_identifier_list_from_food_name =
      Object.keys(markingsDict);
    let menu_line_external_identifiers =
      FoodTL1Parser.getMarkingExternalIdentifiersForMenuLinesFromParsedReportItem(
        parsedReportItem
      );
    let total_marking_external_identifier_list =
      marking_external_identifier_list_from_food_name.concat(
        menu_line_external_identifiers
      );
    return total_marking_external_identifier_list;
  }

  async getRawFoodofferJSONListFromRawReport(
    rawReport: string | Buffer | undefined
  ): Promise<RawFoodofferInformationListType> {
    let jsonListFromCsvString = CSVExportParser.getListOfLineObjectsWithParams(
      rawReport,
      CSVExportParser.NEW_LINE_DELIMITER,
      CSVExportParser.INLINE_DELIMITER_TAB,
      true
    );
    let groupedRawFoodofferItemsFromReport =
      FoodTL1Parser._groupParsedReportItemsToFoodofferListsItems(
        jsonListFromCsvString
      );
    return this.getRawFoodofferJSONListFromGroupedList(
      groupedRawFoodofferItemsFromReport
    );
  }
   ...
   */
  static _groupParsedReportItemsToFoodofferListsItems(
    parsedReportAsJsonList: RawTL1FoodofferType[]
  ): { [p: FoodofferIdentifierType]: RawTL1FoodofferType[] } {
    let dictOfFoodIngredientsForFoodoffer: {
      [p: FoodofferIdentifierType]: { [p: string]: string }[];
    } = {};
    for (let item of parsedReportAsJsonList) {
      let identifier: FoodofferIdentifierType | null =
        FoodTL1Parser.getFoodofferIdentifier(item);
      if (!!identifier) {
        let listOfParsedItemsForSameFoodoffer =
          dictOfFoodIngredientsForFoodoffer[identifier] || [];
        listOfParsedItemsForSameFoodoffer.push(item);
        dictOfFoodIngredientsForFoodoffer[identifier] =
          listOfParsedItemsForSameFoodoffer;
      }
    }
    return dictOfFoodIngredientsForFoodoffer;
  }

  getRawFoodofferJSONListFromGroupedList(groupedReportItems: {
    [p: FoodofferIdentifierType]: RawTL1FoodofferType[];
  }): RawFoodofferInformationListType {
    let foodOfferJSONList: RawFoodofferInformationType[] = [];
    let keys: FoodofferIdentifierType[] = Object.keys(
      groupedReportItems
    ) as FoodofferIdentifierType[];
    for (let key of keys) {
      let listOfItemsForSameFoodoffer = groupedReportItems[key];
      let foodOfferJSON = this.getRawFoodofferInformationFromGroupedItems(
        listOfItemsForSameFoodoffer
      );
      if (!!foodOfferJSON) {
  ...
  static getRecipeIdsFromRawTL1Foodoffer(
    listOfItemsForSameFoodoffer: RawTL1FoodofferType[] | undefined
  ): null | string[] {
    if (
      !listOfItemsForSameFoodoffer ||
      listOfItemsForSameFoodoffer.length === 0
    ) {
      return null;
    }
  ...
  getCombinedSortedRecipeIdAsString(
    listOfItemsForSameFoodoffer: RawTL1FoodofferType[] | undefined
  ): null | string {
    const recipe_ids = FoodTL1Parser.getRecipeIdsFromRawTL1Foodoffer(
      listOfItemsForSameFoodoffer
    );
    if (!recipe_ids) {
      return null;
    ...
    let combined_sorted_recpie_ids =
      FoodTL1Parser.getSortedRecipeIdFromListOfRecipeIds(recipe_ids);
  ...
  getFoodId(
    ...
    let sorted_recipe_ids = this.getCombinedSortedRecipeIdAsString(
      listOfItemsForSameFoodoffer
    );
  ...
  getRawFoodofferInformationFromGroupedItems(
    listOfItemsForSameFoodoffer: RawTL1FoodofferType[] | undefined
  ): RawFoodofferInformationType | null {
    if (
      !listOfItemsForSameFoodoffer ||
      listOfItemsForSameFoodoffer.length === 0
    ) {
   ...
   */
  static getSortedRecipeIdFromListOfRecipeIds(
    string_recipe_ids: string[] | number[]
  ) {
  ...
  static getFoodIdToRawFoodofferDict(
    rawFoodofferJSONList: RawFoodofferInformationListType
  ) {
  ...
  static getParsedReportItemFromrawFoodoffer(
    rawFoodoffer: RawFoodofferInformationType
  ) {
  ...
  static getFoodofferIdentifier(
    parsedReportItem: RawTL1FoodofferType
  ): FoodofferIdentifierType | null {
    if (FoodTL1Parser._hasValidName(parsedReportItem)) {
      ...
      foodofferIdentifier +=
        FoodTL1Parser._getFoodIdentifierByName(parsedReportItem);
  ...
  static getDirectusDateFunction(
    parsedReportItem: RawTL1FoodofferType
  ): FoodofferDateType | null {
    /**
     *   "DATUM": "25.01.2022",
     */
  ...
  static findFirstPriceValueForFields(
    parsedReportItem: RawTL1FoodofferType,
    fields: string[]
  ) {
  ...
  static getPriceForGroup(
    parsedReportItem: RawTL1FoodofferType,
    groupName: PriceGroupEnum
  ): number | null | undefined {
    let foundPrice = null;
    switch (groupName) {
      case PriceGroupEnum.PRICE_GROUP_STUDENT:
        foundPrice = FoodTL1Parser.findFirstPriceValueForFields(
          parsedReportItem,
          [
            FoodTL1Parser.FIELD_PRICE_STUDENT_OSNABRUECK,
            FoodTL1Parser.FIELD_PRICE_STUDENT_HANNOVER,
          ]
        );
        break;
      case PriceGroupEnum.PRICE_GROUP_EMPLOYEE:
        foundPrice = FoodTL1Parser.findFirstPriceValueForFields(
          parsedReportItem,
          [
            FoodTL1Parser.FIELD_PRICE_EMPLOYEE_OSNABRUECK,
            FoodTL1Parser.FIELD_PRICE_EMPLOYEE_HANNOVER,
          ]
        );
        break;
      case PriceGroupEnum.PRICE_GROUP_GUEST:
        foundPrice = FoodTL1Parser.findFirstPriceValueForFields(
          parsedReportItem,
          [
            FoodTL1Parser.FIELD_PRICE_GUEST_OSNABRUECK,
            FoodTL1Parser.FIELD_PRICE_GUEST_HANNOVER,
          ]
        );
        break;
      default:
  ...
  static getAdditionalFoodAttributesFromRawTL1Foodoffer(
    parsedReportItem: RawTL1FoodofferType,
    csvAttributes: Tl1AttributeType[]
  ): FoodParseFoodAttributesType {
  ...
  getFoodAttributesFromRawTL1Foodoffer(
    parsedReportItem: RawTL1FoodofferType
  ): FoodParseFoodAttributesType {
    let foodAttributes: FoodParseFoodAttributesType = [];
    let nutritionAttributes =
      FoodTL1Parser.getFoodNutritionAttributeValuesFromRawTL1Foodoffer(
        parsedReportItem
      );
  ...
  static getFoodNutritionAttributeValuesFromRawTL1Foodoffer(
    parsedReportItem: RawTL1FoodofferType
  ): FoodParseFoodAttributesType {
    let attributeValues: FoodParseFoodAttributesType = [];
     ...
     */
    let nutritionValuesString =
      parsedReportItem[FoodTL1Parser.DEFAULT_NUTRITIONS_FIELD];
    if (!!nutritionValuesString) {
      ...
      attributeValues.push({
        ...
        attribute_value: {
          number_value: FoodTL1Parser.parseNutritionValue(
            nutritionValuesString,
            'Fett'
          ),
        },
      });
  ...
  static getFoodEnvironmentImpactAttributeValuesFromRawTL1Foodoffer(
    parsedReportItem: RawTL1FoodofferType
  ): FoodParseFoodAttributesType {
    let attributeValues: FoodParseFoodAttributesType = [];
    ...
    let co2_saving_percentage =
      parsedReportItem[FoodTL1Parser.DEFAULT_CO2_SAVING_PERCENTAGE_FIELD];
  ...
  static parseNutritionValue(
    nutritionValuesString: string,
    searchValue: string
  ) {
    try {
  ...
  static getMarkingExternalIdentifiersForMenuLinesFromParsedReportItem(
    parsedReportItem: RawTL1FoodofferType
  ): string[] {
    let menu_line_text =
      parsedReportItem[FoodTL1Parser.DEFAULT_MENU_LINE_FIELD];
    return FoodTL1Parser.getMarkingExternalIdentifierForMenuLineText(
      menu_line_text
    );
  }

  static getMarkingExternalIdentifierForMenuLineText(
    menu_line_text: string | undefined
  ): string[] {
    ...
    if (!!menu_line_text && menu_line_text.length > 0) {
      ...
      for (let menu_line of menu_lines) {
        ...
        if (!!menu_line_trimmed && menu_line_trimmed.length > 0) {
          menu_lines_with_prefix.push(
            FoodTL1Parser.MARKING_EXTERNAL_IDENTIFIER_PREFIX_FOR_MENU_LINE +
              menu_line_trimmed
          );
        }
      }
  ...
  static getMarkingLabelsDictFromFoodName(name: string) {
    ...
    if (!!rawMarkingsInName) {
      for (let rawMarkingsPart of rawMarkingsInName) {
        //e. G. "(g, b)"
        let listOfPartMarkings =
          FoodTL1Parser.removeValuesAndWhitespacesAndSeperators(
            rawMarkingsPart,
            ['(', ')'],
            ','
          );
  ...
  static removeValuesAndWhitespacesAndSeperators(
    string: string,
  ...
  static _getRawNamesList(
    ...
    for (let i = 1; i <= 6; i++) {
      let partialName =
        parsedReportItem[FoodTL1Parser.DEFAULT_TEXT_FIELD + i + postFieldName];
  ...
  static _getFoodNameEn(parsedReportItem: RawTL1FoodofferType) {
    let rawFoodName = FoodTL1Parser._getRawNamesList(
      parsedReportItem,
      '_1'
    ).join(', ');
    return FoodTL1Parser.sanitizeFoodNameFromMarkingLabels(rawFoodName);
