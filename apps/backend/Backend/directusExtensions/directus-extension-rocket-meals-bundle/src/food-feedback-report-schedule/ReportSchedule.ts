...
const TABLENAME_CANTEEN_FOOD_FEEDBACK_REPORT_SCHEDULES =
...
export class ReportSchedule {
  ...
  async run() {
    ...
    try {
      ...
      for (let reportSchedule of reportSchedules) {
        ...
        let referenceDateForReport =
          await this.getReferenceDateOfTheReportOrNull(reportSchedule);
        ...
        if (referenceDateForReport) {
          ...
          let startDate = ReportSchedule.getStartDateBasedOnReferenceDate(
            referenceDateForReport,
            reportSchedule
          );

          let recipientEmailList =
            await this.getRecipientEmailList(reportSchedule);
          ...
          if (recipientEmailList.length > 0) {
            ...
            if (Object.keys(canteensDict).length > 0) {
              try {
                ...
                let generated_report_data: ReportType =
                  await reportGenerator.generateReportJSON(
                    reportSchedule,
                    startDate,
                    endDate,
                    canteensDict
                ...
                if (!!generated_report_data) {
                  ...
                  await this.updateReportLogSuccess(
                    referenceDateForReport,
                    reportSchedule
                  );
                } else {
                  await this.logReportSendError(
                    reportSchedule,
                    'No report could be generated. Please contact admin and tell him: await reportGenerator.generateReportForMail(referenceDateForReport)'
                  );
                }
              } catch (err) {
  ...
  static getStartDateBasedOnReferenceDate(
    referenceDate: Date,
    reportSchedule: Partial<DatabaseTypes.CanteenFoodFeedbackReportSchedules>
  ): Date {
    ...
    let foodoffers_days_limit =
      reportSchedule.period_days_amount || DEFAULT_FOODOFFERS_DAYS_LIMIT;
  ...
  async getRecipientEmailList(
    ...
    let report_schedule_report_recipients_service =
      await itemsServiceCreator.getItemsService<DatabaseTypes.CanteenFoodFeedbackReportSchedulesReportRecipients>(
        CollectionNames.CANTEEN_FOOD_FEEDBACK_REPORT_SCHEDULES_REPORT_RECIPIENTS
      );

    let report_schedule_report_recipients =
      await report_schedule_report_recipients_service.readByQuery({
        filter: {
    ...
    let report_recipients_service =
      await itemsServiceCreator.getItemsService<DatabaseTypes.ReportRecipients>(
        CollectionNames.REPORT_RECIPIENTS
      );
    let report_recipients = await report_recipients_service.readMany(
      report_recipients_primary_keys
    );
  ...
  async getCanteenEntries(
    recipientEntry: DatabaseTypes.CanteenFoodFeedbackReportSchedules
  ): Promise<Record<string, DatabaseTypes.Canteens>> {
    const itemsServiceCreator = new ItemsServiceCreator(this.apiContext);
    let itemService =
      await itemsServiceCreator.getItemsService<DatabaseTypes.CanteenFoodFeedbackReportSchedulesCanteens>(
    ...
    let allCanteens = await this.myDatabaseHelper
      .getCanteensHelper()
      .readAllItems();
    ...
    let canteenService =
      await itemsServiceCreator.getItemsService<DatabaseTypes.Canteens>(
        CollectionNames.CANTEENS
      );
    let canteensList = await canteenService.readMany(canteen_primary_keys);
    let canteensAsDict = DictHelper.transformListToDict(
      canteensList,
      canteen => canteen.id
    );
  ...
  private getCanteenAliasForMail(
  ...
  async sendReport(
    generated_report_data: ReportType,
    recipientEntry: DatabaseTypes.CanteenFoodFeedbackReportSchedules,
  ...
  async setNextReportDate(
    ...
    let itemService =
      await itemsServiceCreator.getItemsService<DatabaseTypes.CanteenFoodFeedbackReportSchedules>(
        tablename
      );
    // update when the next report is due
    let now = new Date();
    let new_date_next_report_is_due_iso =
      ReportSchedule.getNextReportIsDueDateIsoOrNull(reportSchedule, now);

    if (!new_date_next_report_is_due_iso) {
      let messageNoSuitableNextReportDateFound =
        'No suitable next report date found. Please select at least one weekday.';
      ...
      if (
        !reportSchedule.report_status_log ||
        reportSchedule.report_status_log !==
          messageNoSuitableNextReportDateFound
      ) {
        await this.updateReportLog(
          reportSchedule,
          messageNoSuitableNextReportDateFound,
          false
        );
      ...
      await this.updateReportLog(
        reportSchedule,
        'Next report date was not set. Set next report due date to: ' +
          new_date_next_report_is_due_iso,
        true
      );
    }
  }

  async updateReportLogSuccess(
  ...
  async logReportSendError(
    ...
    await this.updateReportLog(
      recipientEntry,
      'Report sending failed: ' + err.toString(),
      false
    );
  }

  async updateReportLog(
    recipientEntry: DatabaseTypes.CanteenFoodFeedbackReportSchedules,
    log: string,
    success: boolean
  ) {
  ...
  async getReferenceDateOfTheReportOrNull(
    recipientEntry: DatabaseTypes.CanteenFoodFeedbackReportSchedules
  ) {
    //console.log("#############");
    ...
    let send_once_now_for_date =
      recipientEntry?.send_once_now_for_reference_date;
    ...
    let current_date_next_report_is_due =
      recipientEntry.date_next_report_is_due;
  ...
  public static getReferenceDate(
    recipientEntry: Partial<DatabaseTypes.CanteenFoodFeedbackReportSchedules>,
    now: Date
  ): Date | null {
    ...
    let date_for_which_the_report_should_be_generated = moment(
      now_moment_date.toISOString()
    )
      .add(foodoffers_days_offset, 'days')
      .set({
        hour: 12,
        minute: 0,
        second: 0,
      });
    let date_for_which_the_report_should_be_generated_iso =
      date_for_which_the_report_should_be_generated.toISOString();
    return new Date(date_for_which_the_report_should_be_generated_iso);
  }

  public static splitSendReportAtHhMm(
    recipientEntry: Partial<DatabaseTypes.CanteenFoodFeedbackReportSchedules>
  ) {
  ...
  public static getNextReportIsDueToBeGeneratedDateOrNull(
    reportSchedule: Partial<DatabaseTypes.CanteenFoodFeedbackReportSchedules>,
    now: Date
  ): Date | null {
    if (!reportSchedule.enabled) {
    ...
    const send_report_at_splits =
      ReportSchedule.splitSendReportAtHhMm(reportSchedule);
    ...
    let date_when_the_next_report_should_be_generated = moment(now_copy)
      .add(0, 'days')
      .set({
    ...
    if (
      now_moment_date.isAfter(date_when_the_next_report_should_be_generated)
    ) {
      ...
      date_when_the_next_report_should_be_generated = moment(
        date_when_the_next_report_should_be_generated.toISOString()
      ).add(1, 'days');
    } else {
    ...
    //console.log("date_for_which_the_report_should_be_generated_moment_date: "+date_for_which_the_report_should_be_generated_moment_date)
    const next_weekdayList_for_date_for_which_the_report_should_be_generated =
      DateHelper.getWeekdayListFromDate(
        new Date(date_when_the_next_report_should_be_generated.toISOString())
      );
    ...
    let suitable_date_when_the_next_report_should_be_generated_moment_date =
      moment(date_when_the_next_report_should_be_generated.toISOString()).add(
        amount_days_to_add,
        'days'
      );
    suitable_date_when_the_next_report_should_be_generated_moment_date =
      suitable_date_when_the_next_report_should_be_generated_moment_date.set({
        hour: send_report_at_hh,
        minute: send_report_at_mm,
        second: send_report_at_ss,
      });

    let date_next_report_is_due_iso =
      suitable_date_when_the_next_report_should_be_generated_moment_date.toISOString();
  ...
  public static getNextReportIsDueDateIsoOrNull(
    recipientEntry: Partial<DatabaseTypes.CanteenFoodFeedbackReportSchedules>,
    now: Date
  ): string | null {
    let nextReportIsDueDate =
  ...
  public static haveTimeSettingsChanged(
    ...
    const fieldsToCheck: Array<
      keyof DatabaseTypes.CanteenFoodFeedbackReportSchedules
    > = [
      'enabled',
    ...
    return fieldsToCheck.some(field => {
      ...
      if (
        newCanteenFoodFeedbackReportSchedules[
          field as keyof DatabaseTypes.CanteenFoodFeedbackReportSchedules
        ] !== undefined
      ) {
        const currentValue =
          currentCanteenFoodFeedbackReportSchedules[
            field as keyof DatabaseTypes.CanteenFoodFeedbackReportSchedules
          ];
        const newValue =
          newCanteenFoodFeedbackReportSchedules[
