...
export default defineHook(async ({ schedule, filter }, apiContext) => {
  ...
  filter<Partial<DatabaseTypes.CanteenFoodFeedbackReportSchedules>>(
    collection + '.items.update',
    async (
      modifiable_payload: Partial<DatabaseTypes.CanteenFoodFeedbackReportSchedules>,
      meta,
      eventContext
    ) => {
      ...
      const doesModifiablePayloadHasDateNextReportIsDueSet =
        !!modifiable_payload.date_next_report_is_due;
      ...
      if (!doesModifiablePayloadHasDateNextReportIsDueSet) {
        ...
        for (const key of keys) {
          ...
          const currentItem =
            await reportSchedule.getCanteenFoodFeedbackReportScheduleById(key);
          if (!!currentItem) {
            //console.log(pre+"checking if time settings have changed");
            const haveTimeSettingsChanged =
              ReportSchedule.haveTimeSettingsChanged(
                currentItem,
                modifiable_payload
              );
