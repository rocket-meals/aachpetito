...
export class DirectusCollectionTranslator {
  ...
  static areTranslationsToTranslate(payload: any, translation_field: string) {
    ...
    if (!!payload && !!payload[translation_field]) {
      ...
      return (
        newTranslationsCreateActions.length > 0 ||
        newTranslationsUpdateActions.length > 0
      );
    }
    return false;
  }

  static getSourceTranslationFromTranslations(
    translations: any,
    ...
    if (!!translations && translations.length > 0) {
      for (let translation of translations) {
        let let_be_source_for_translation =
          DirectusCollectionTranslator.getValueFromPayloadOrDefaultValue(
            translation,
            DirectusCollectionTranslator.FIELD_BE_SOURCE_FOR_TRANSLATION,
            schema,
            collectionName,
            translation_field
          );
  ...
  static getSourceTranslationFromListsOfTranslations(
    listsOfTranslations: any,
    schema: SchemaOverview,
    collectionName: string,
    translation_field: string
  ) {
    if (!!listsOfTranslations && listsOfTranslations.length > 0) {
      for (let i = 0; i < listsOfTranslations.length; i++) {
        ...
        let sourceTranslation =
          DirectusCollectionTranslator.getSourceTranslationFromTranslations(
            translations,
            schema,
            collectionName,
            translation_field
  ...
  static setFIELD_LANGUAGES_ID_OR_CODE(translation: any): string | undefined {
    const translationFieldOld =
      translation?.[DirectusCollectionTranslator.FIELD_LANGUAGES_CODE_OLD];
    if (!!translationFieldOld) {
      DirectusCollectionTranslator.FIELD_LANGUAGES_ID_OR_CODE =
        DirectusCollectionTranslator.FIELD_LANGUAGES_CODE_OLD;
      return DirectusCollectionTranslator.FIELD_LANGUAGES_CODE_OLD;
    }
    const translationFieldNew =
      translation?.[DirectusCollectionTranslator.FIELD_LANGUAGES_IDS_NEW];
    if (!!translationFieldNew) {
      DirectusCollectionTranslator.FIELD_LANGUAGES_ID_OR_CODE =
        DirectusCollectionTranslator.FIELD_LANGUAGES_IDS_NEW;
  ...
  static async modifyPayloadForTranslation(
    currentItem: any,
    payload: any,
    ...
    if (
      DirectusCollectionTranslator.areTranslationsToTranslate(
        payload,
        translation_field
      )
    ) {
      ...
      for (let translation of currentTranslations) {
        ...
        let FIELD_LANGUAGES_ID_OR_CODE =
          DirectusCollectionTranslator.setFIELD_LANGUAGES_ID_OR_CODE(
            translation
          );
        ...
        if (!!FIELD_LANGUAGES_ID_OR_CODE) {
          existingTranslations[translation?.[FIELD_LANGUAGES_ID_OR_CODE]] =
            translation;
        }
      }
      ...
      let newTranslationsCreateLanguageDict =
        DirectusCollectionTranslator.parseTranslationListToLanguagesCodeDict(
          newTranslationsCreateActions
        );
      let newTranslationsUpdateLanguageDict =
        DirectusCollectionTranslator.parseTranslationListToLanguagesCodeDict(
          newTranslationsUpdateActions
        );

      let sourceTranslationInExistingItem =
        DirectusCollectionTranslator.getSourceTranslationFromListsOfTranslations(
          [currentTranslations],
          schema,
          collectionName,
          translation_field
        );
      let sourceTranslationInPayload =
        DirectusCollectionTranslator.getSourceTranslationFromListsOfTranslations(
          [newTranslationsCreateActions, newTranslationsUpdateActions],
          schema,
          collectionName,
          translation_field
        );
      ...
      let sourceTranslation =
        sourceTranslationInPayload || sourceTranslationInExistingItem;
      /**
       * From Singleton
       * sourceTranslation:
       * {
      ...
      if (sourceTranslation) {
        ...
        if (!!FIELD_LANGUAGES_ID_OR_CODE) {
          sourceTranslationLanguageCode =
            sourceTranslation?.[FIELD_LANGUAGES_ID_OR_CODE]?.code;
        }
        //console.log("sourceTranslationLanguageCode: ", sourceTranslationLanguageCode);

        let languagesService =
          myDatabaseHelper.getItemsServiceHelper<DatabaseTypes.Languages>(
            DirectusCollectionTranslator.COLLECTION_LANGUAGES
          );
        ...
        if (languages.length > 0 && !!FIELD_LANGUAGES_ID_OR_CODE) {
          ...
          let fieldsToTranslate =
            DirectusCollectionTranslator.getFieldsToTranslate(
              schema,
              collectionName,
              translation_field
            );
          ...
          for (let language of languages) {
            ...
            let isSourceTranslation =
              language_code === sourceTranslationLanguageCode;
            ...
            if (!!existingTranslation) {
              ...
              if (isSourceTranslation) {
                ...
                let letBeTranslatedInExistingTranslation =
                  existingTranslation?.[
                    DirectusCollectionTranslator.FIELD_LET_BE_TRANSLATED
                  ];
                ...
                if (
                  DirectusCollectionTranslator.isValueDefined(
                    letBeTranslatedInPayload
                  )
                ) {
                ...
                if (!!createTranslation) {
                  ...
                  let translatedItem =
                    await DirectusCollectionTranslator.translateTranslationItem(
                      sourceTranslation,
                      language_code,
                      translator,
                      translatorSettings,
                  ...
                  translationsToUpdate.push({
                    ...translationInPayload,
                    [DirectusCollectionTranslator.FIELD_BE_SOURCE_FOR_TRANSLATION]:
                      false, //but we dont want it to be the source translation anymore
                  });
                } else {
              ...
              if (isSourceTranslation) {
                ...
                if (letBeTranslated) {
                  ...
                  translationsToCreate.push({
                    ...translationInPayload,
                    [DirectusCollectionTranslator.FIELD_BE_SOURCE_FOR_TRANSLATION]:
                      false, //but we dont want it to be the source translation
                  });
                } else {
                  //console.log("No payload given for this language");
                }
  ...
  static getValueFromPayloadOrDefaultValue(
    payloadItem: any,
    fieldName: string,
    schema: any,
    collectionName: string,
    ...
    let translationCollectionSchema =
    ...
    if (DirectusCollectionTranslator.isValueDefined(valueInPayload)) {
      ...
      let defaultValue =
        translationCollectionSchema?.fields?.[fieldName]?.defaultValue;
      return defaultValue;
    }
  }

  static async translateTranslationItem(
    ...
    if (!!fieldsToTranslate && fieldsToTranslate.length > 0) {
      for (let field of fieldsToTranslate) {
        ...
        if (!!fieldValue) {
          try {
            let translatedValue = await translator.translate(
              fieldValue,
              sourceTranslation?.[FIELD_LANGUAGES_ID_OR_CODE]?.code,
              language_code
            );
    ...
    translatedItem[
      DirectusCollectionTranslator.FIELD_BE_SOURCE_FOR_TRANSLATION
    ] = false; //if translated it wont be the source translation anymore
    return translatedItem;
  }

  static getTranslationCollectionName(
    schema: any,
    collectionName: string,
    translation_field: string
  ): string | null {
    ...
    for (let relation of relations) {
      ...
      if (relation?.related_collection === collectionName) {
        ...
        if (
          metaOneCollection === collectionName &&
          metaOneField === translation_field
        ) {
  ...
  static getTranslationCollectionSchema(
    schema: any,
    collectionName: string,
    translation_field: string
  ) {
    let translationCollectionName =
      DirectusCollectionTranslator.getTranslationCollectionName(
        schema,
        collectionName,
        translation_field
      );
    ...
    let collectionInformations =
      DirectusCollectionTranslator.getSchemaForCollection(
        schema,
  ...
  static getFieldsToTranslate(
     ...
     * ]
     */
    let translationCollectionInformations =
      DirectusCollectionTranslator.getTranslationCollectionSchema(
        schema,
        collectionName,
        translation_field
      );
    let collectionFieldsInformationsDict =
      translationCollectionInformations?.fields || {};
    ...
    for (let field of collectionFields) {
      if (field !== primaryFieldKey) {
        ...
        if (
          fieldsInformation?.type === 'text' ||
          fieldsInformation?.type === 'string'
        ) {
