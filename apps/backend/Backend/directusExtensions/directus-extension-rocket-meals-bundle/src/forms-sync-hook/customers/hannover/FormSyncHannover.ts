...
export class FormSyncHannover {
  public static registerHooks(
    triggerContext: RegisterFunctions,
    apiContext: ApiExtensionContext
  ): void {
    const myDatabaseHelper = new MyDatabaseHelper(apiContext);
    let dockerVolumeMountedPath =
      EnvVariableHelper.getHousingContractCsvFilePath();
    console.log(
      'FormSyncHannover - Housing dockerVolumeMountedPath: ',
      dockerVolumeMountedPath
    );
    if (!dockerVolumeMountedPath || dockerVolumeMountedPath === '') {
      console.error(
        'Missing env variable: HOUSING_CONTRACT_SYNC_TL1FILE_EXPORT_CSV_FILE_PATH'
      );
      return;
    }
    ...
    WorkflowScheduleHelper.registerScheduleToRunWorkflowRuns({
      workflowRunInterface: new FormHousingContractsWorkflowHannover(
        DIRECTUS_TL1_HOUSING_EXPORT_PATH
      ),
