...
function registerHookPresentCreateFormSubmissionIllegalState(
  ...
  registerFunctions.filter<Partial<DatabaseTypes.FormSubmissions>>(
    ...
    async (input, meta, eventContext) => {
      if (input.state !== FormSubmissionState.DRAFT && !!input.state) {
        ...
        throw new Error(
          'Only form submissions with state draft are allowed to be created.'
        );
      }
      return input;
    }
...
function registerHookPreventUpdateFormSubmissionIllegalState(
  registerFunctions: RegisterFunctions,
  apiContext: ApiContext
) {
  registerFunctions.filter<Partial<DatabaseTypes.FormSubmissions>>(
    CollectionNames.FORM_SUBMISSIONS + '.items.update',
    async (input, meta, eventContext) => {
      ...
      for (let form_submission_id of form_submission_ids) {
        let formSubmission = await myDatabaseHelper
          .getFormsSubmissionsHelper()
          .readOne(form_submission_id);
        if (formSubmission.state === FormSubmissionState.SYNCING) {
          if (!!input.state) {
            if (
              input.state === FormSubmissionState.CLOSED ||
              input.state === FormSubmissionState.FAILED
            ) {
              // Only allow to set the state to closed or failed
            } else {
              throw new Error(
                'Form submission is in state syncing. It is not allowed to update the form submission. Please set the state to closed.'
              );
            }
          }
        }
...
function registerHookHandleFormSubmissionDateSubmitted(
  registerFunctions: RegisterFunctions,
  apiContext: ApiContext
) {
...
function registerHookCheckAllRequiredFieldsAreFilled(
  registerFunctions: RegisterFunctions,
  apiContext: ApiContext
) {
    ...
    async (input, meta, eventContext) => {
      ...
      if (input.state === FormSubmissionState.SUBMITTED) {
        ...
        for (let form_submission_id of form_submission_ids) {
          ...
          let form_answers = await myDatabaseHelper
          ...
          for (let required_form_field of required_form_fields) {
            ...
            let form_answer = form_answers.find(
              fa => fa.form_field === form_field_id
            );
            if (!form_answer) {
              throw new Error(
                'Required form answer not found for form field with alias: ' +
                  required_form_field.alias +
                  ' (id: ' +
                  required_form_field.id +
                  ')'
              );
            }
            let value_string_is_set =
              form_answer.value_string !== null &&
              form_answer.value_string !== undefined;
            let value_number_is_set =
              form_answer.value_number !== null &&
              form_answer.value_number !== undefined;
            let value_date_is_set =
              form_answer.value_date !== null &&
              form_answer.value_date !== undefined;
            let value_boolean_is_set =
              form_answer.value_boolean !== null &&
              form_answer.value_boolean !== undefined;
            let value_custom_is_set =
              form_answer.value_custom !== null &&
              form_answer.value_custom !== undefined;
            let value_files_is_set =
              form_answer.value_files !== null &&
              form_answer.value_files !== undefined;
            let value_image_is_set =
              form_answer.value_image !== null &&
              form_answer.value_image !== undefined;
            let value_is_set =
              value_string_is_set ||
              value_number_is_set ||
              value_date_is_set ||
              value_boolean_is_set ||
              value_custom_is_set ||
              value_files_is_set ||
              value_image_is_set;
            if (!value_is_set) {
              throw new Error(
                'Please fill out the required form field with alias: ' +
                  required_form_field.alias +
                  ' (id: ' +
                  required_form_field.id +
                  ')'
...
function registerHookSetStateToSynchingAfterFormSubmission(
  registerFunctions: RegisterFunctions,
  apiContext: ApiContext
) {
    ...
    async (meta, context) => {
      ...
      for (let form_submission_id of form_submission_ids) {
        ...
        if (formSubmission.state === FormSubmissionState.SUBMITTED) {
          console.log(
            'Set state to syncing for form submission id: ' + form_submission_id
          );
          await myDatabaseHelper
            .getFormsSubmissionsHelper()
            .updateOne(form_submission_id, {
...
export type FormExtractFormAnswerValueFileSingleOrString =
  | FormExtractFormAnswerValueFileSingle
  | string;
export type FormExtractFormAnswer = Omit<
  DatabaseTypes.FormAnswers,
  'value_image' | 'value_files'
> & {
...
export type FormExtractRelevantInformation =
  FormExtractRelevantInformationSingle[];

function registerHookSendMailAfterFormSubmissionStateSyncing(
  registerFunctions: RegisterFunctions,
  apiContext: ApiContext
) {
      ...
      for (let form_submission_id of form_submissions_ids) {
        ...
        if (formSubmission.state === FormSubmissionState.SYNCING) {
          try {
            ...
            let form_with_translations = await myDatabaseHelper
              .getFormsHelper()
              .readOneWithTranslations(form_id);

            // Get the answers of the user
            console.log('Get form answers');
            let form_answers_raw = await myDatabaseHelper
              .getFormsAnswersHelper()
              .findItems(
                {
                  form_submission: formSubmission.id,
            ...
            let form_answers: FormExtractFormAnswer[] =
              form_answers_raw as FormExtractFormAnswer[];
            ...
            let formExtracts = await myDatabaseHelper
            ...
            for (let form_extract of formExtracts) {
              ...
              if (recipient_email_field_id) {
                let form_answer_recipient_email = form_answers.find(
                  fa => fa.form_field === recipient_email_field_id
                );
                if (!form_answer_recipient_email) {
                  throw new Error(
                    'Recipient email field not found for id: ' +
                      recipient_email_field_id
                  );
                }
                dynamic_email_dynamic =
                  form_answer_recipient_email.value_string;
              ...
              if (recipient_user_raw) {
                ...
                if (typeof recipient_user_raw === 'string') {
                  let user = await myDatabaseHelper
                    .getUsersHelper()
                    .readOne(recipient_user_raw);
                  if (!user) {
                    throw new Error(
                      'User not found for id: ' + recipient_user_raw
                    );
                  }
                  recipient_user = user;
                }
              ...
              if (!atleast_one_recipient_email) {
                throw new Error(
                  'No recipient email found for form extract id: ' +
                    form_extract_id
                );
              }

              console.log(
                'Get form_extract_form_fields for form_extract id: ' +
                  form_extract_id
              );
              ...
              if (form_extract.all_fields) {
                ...
                let formExtractFields = await myDatabaseHelper
                ...
                for (let formExtractField of formExtractFields) {
                  ...
                  let form_field = form_fields.find(
                    ff => ff.id === form_field_id
                  );
                  if (!form_field) {
                    throw new Error(
                      'Form field not found for id: ' + form_field_id
                    );
                  }
              ...
              for (let relevant_form_field of relevant_form_fields) {
                ...
                if (!form_answer) {
                  throw new Error(
                    'Form answer not found for form field id: ' + form_field_id
                  );
                }
              ...
              await sendFormExtractMail(
                form_with_translations,
                form_extract,
                formSubmission,
                form_answers_relevant_for_form_extract,
                recipient_emails,
                myDatabaseHelper
              );
            }
            ...
            await myDatabaseHelper
              .getFormsSubmissionsHelper()
              .updateOneItemWithoutHookTrigger(
                {
            ...
            console.error(
              'Error while sending mail after form submission state syncing: ' +
                e.toString()
            );
            console.error(e);
          }
...
async function sendFormExtractMail(
  ...
  let form_translated_name = TranslationHelper.getTranslation(
    form.translations,
  ...
  for (let recipient_email of recipient_emails) {
    ...
    let newFile = await myDatabaseHelper
      .getFilesHelper()
      .uploadOneFromBuffer(
        pdfBuffer,
        form_name + '.pdf',
    ...
    let mail: Partial<DatabaseTypes.Mails> = {
      ...
      markdown_content:
        'Anbei finden Sie eine Kopie des Formulars: ' + form_name + '\n\n',
      subject: subject,
...
export default defineHook(async (registerFunctions, apiContext) => {
  ...
  registerHookToCreateFormAnswersForFormSubmission(
    registerFunctions,
    apiContext
  );

  // Allow only drafts to be created
  registerHookPresentCreateFormSubmissionIllegalState(
    registerFunctions,
    apiContext
  );
  ...
  registerHookPreventUpdateFormSubmissionIllegalState(
    registerFunctions,
    apiContext
  );

  // Set the state to syncing after form submission
  registerHookSetStateToSynchingAfterFormSubmission(
    registerFunctions,
    apiContext
  );

  // Send mail after form submission state syncing
  registerHookSendMailAfterFormSubmissionStateSyncing(
    registerFunctions,
    apiContext
  );
