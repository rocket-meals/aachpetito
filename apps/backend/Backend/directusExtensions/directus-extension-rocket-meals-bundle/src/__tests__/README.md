...
A reusable TypeScript class for setting up and managing Directus test servers with in-memory SQLite
databases.

## Overview

The `DirectusTestServerSetup` class encapsulates all the complex server setup logic previously
scattered in test files, providing a clean, reusable interface for Directus testing scenarios.
...
This reduces test setup complexity from ~150 lines to ~5 lines while improving maintainability and
reusability.
